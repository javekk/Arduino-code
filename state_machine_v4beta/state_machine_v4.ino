...
int Stacknum; // holds the value of the stack that needs to dispence the pill
...
SoftwareSerial SIM900(9, 10); //GSM
char number[16] = {};
char awayHour = 'f';
        ...
        case monitorAlarm : 
           ...
           Serial.println ("isAwayModePressed: "+String(isAwayModePressed));
            ...
            case SetbothAlarms    : 
                ...
                if(SetBothAlarm_status == E_OK){
                    //  process_state = pillDispense;
                    Serial.println("Dispense Stck1 + Stck2");
                    process_state = pillDispense;
                    Stacknum = stack1+stack2;
                    alarm_flag = 1;
                }
          ...
          case awayModeDispensePill :
                   ...
                   lcd.clear();
                    ...
                    if(alarmCount_Stack1!=0 && alarmCount_Stack2!=0){
                      Stacknum=stack1+stack2;
                      count1++;
                      count2++;
                      // awayModeDispensePill(Stacknum);
                      Serial.println("Dispense pills for the alarm Stack 1 and Stack 2");
                    }
                    else if((count1!=alarmCount_Stack1)&&(alarmCount_Stack1!=0)){
                      Stacknum=stack1;
                     // awayModeDispensePill(Stacknum);
                      count1++;
                    ...
                    Serial.println("Dispense pills for the alarm Stack 2");
                     ...
                     process_state = awayModeDispensePill;
                      Serial.println("Away Mode is ON now");
...
int func_awayModeMonitorAlarm(int val) {
    ...
    if((hours()<=alarm1_Hours)&&(alarm1_Hours<=val)){
        retval1 = 1;
        Serial.println("Stack1");
    }
    if((hours()<=alarm2_Hours)&&(alarm2_Hours<=val)){
        retval2 = 2;
        Serial.println("Stack2");
    }
    return (retval1+retval2);
